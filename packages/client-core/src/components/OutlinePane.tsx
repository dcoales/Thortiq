...
import {buildRichOverlayStyles} from './richOverlayStyles';
...
import {getCaretOffsetFromPoint} from '../utils/caret';
...
const ENABLE_RICH_EDITOR = false;
...
export const OutlinePane = ({rootId, className}: OutlinePaneProps) => {
  ...
  const [richReadyEdges, setRichReadyEdges] = useState<Set<EdgeId>>(() => new Set());



  const markRichReady = useCallback((edgeId: EdgeId, ready: boolean) => {



    setRichReadyEdges((current) => {



      const hasEdge = current.has(edgeId);
  ...
  useEffect(() => {
    ...
    markRichReady(activeEdgeId, false);



  }, [activeEdgeId, markRichReady]);
  ...
  const handleRichUnderlayPointerDown = useCallback((edge: EdgeRecord, event: MouseEvent<HTMLDivElement>) => {
    ...
    const offset = getCaretOffsetFromPoint(event.currentTarget, point);
    ...
    if (typeof offset === 'number') {
      ...
      issueRichSelectAtOffset(edge.id, offset);
    ...
    issueRichFocusAtPoint(edge.id, point.x, point.y);



  }, [issueRichFocusAtPoint, issueRichSelectAtOffset, markRichReady]);
            ...
            renderNode={(row) => {
              ...
              if (ENABLE_RICH_EDITOR) {
                ...
                if (isActive && row.edge) {
                  ...
                  const activeEdge = row.edge;

                  const richDirective = richFocusRequest && richFocusRequest.edgeId === activeEdge.id
                  ...
                  const selectDirective = richSelectRequest && richSelectRequest.edgeId === activeEdge.id
                  ...
                  const overlayReady = richReadyEdges.has(activeEdge.id);

                  const overlayStyles: ReturnType<typeof buildRichOverlayStyles> = buildRichOverlayStyles(TEXT_LINE_HEIGHT, overlayReady);



                  const underlay = overlayStyles.underlay;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        ...
                        onMouseDown={(event) => handleRichUnderlayPointerDown(activeEdge, event)}
                      ...
                      >
                        ...
                        <span className="thq-node-text" dangerouslySetInnerHTML={{__html: row.node.html}} />
                      ...
                      >
                        ...
                        <RichNodeEditor
                          ...
                          onReady={() => markRichReady(activeEdge.id, true)}



                          onDispose={() => markRichReady(activeEdge.id, false)}
                ...
                return (
                  ...
                  <div
                    ...
                    onMouseDown={(event) => {
                      ...
                      handleRichUnderlayPointerDown(row.edge, event);
