...
describe("OutlineRowView", () => {
  ...
  it("shows static text when the pane is inactive even if the editor was attached", () => {
    const { container } = render(
      <OutlineRowView
        row={createRow()}
        isSelected={true}
        ...
        editorEnabled={true}
        editorAttachedEdgeId="edge-root"
        isActivePane={false}
        presence={[]}
        dropIndicator={null}
        onSelect={vi.fn()}
    ...
    const staticContent = container.querySelector('[data-outline-text-content="true"]') as HTMLElement | null;
    expect(staticContent).not.toBeNull();
    expect(staticContent?.style.display).toBe("inline");
  });
